...
@Serializable data class ExplicitlyNamedStruct(val a_field: String, val another_field: UInt)
...
sealed interface SomeEnum {
    ...
    @Serializable @SerialName("B") data class B(val field1: String) : SomeEnum

    @Serializable @SerialName("C") data class C(val field1: UInt, val field2: Float) : SomeEnum

    @Serializable @SerialName("D") data class D(val field3: Boolean? = null) : SomeEnum

    @Serializable @SerialName("E") data class E(val value: ExplicitlyNamedStruct) : SomeEnum
    ...
    object Serializer : KSerializer<SomeEnum> {
        ...
        override fun serialize(encoder: Encoder, value: SomeEnum) {
            ...
            when (value) {
                ...
                is B -> {
                    val content =
                            encoder.json.encodeToJsonElement(B.serializer(), value) as JsonObject
                ...
                is C -> {
                    val content =
                            encoder.json.encodeToJsonElement(C.serializer(), value) as JsonObject
                ...
                is D -> {
                    val content =
                            encoder.json.encodeToJsonElement(D.serializer(), value) as JsonObject
                ...
                is E -> {
                    val content =
                            encoder.json.encodeToJsonElement(
                                    ExplicitlyNamedStruct.serializer(),
                                    value.value
                            ) as
                                    JsonObject
        ...
        override fun deserialize(decoder: Decoder): SomeEnum {
            ...
            return when (val discriminator = container.jsonObject["type"]?.jsonPrimitive?.content) {
                ...
                "E" -> {
                    val content =
                            decoder.json.decodeFromJsonElement<ExplicitlyNamedStruct>(container)
                    E(content)
                }
