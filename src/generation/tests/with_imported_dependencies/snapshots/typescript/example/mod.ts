...
constructor (public external: ExternalParent) {
}
...
static deserialize(deserializer: Deserializer): Child {
  const external = ExternalParent.deserialize(deserializer);
...
static deserialize(deserializer: Deserializer): ExternalChild {
  const name = deserializer.deserializeStr();
  return new ExternalChild(name);
}

}
export abstract class ExternalParent {
abstract serialize(serializer: Serializer): void;

static deserialize(deserializer: Deserializer): ExternalParent {
  const index = deserializer.deserializeVariantIndex();
  switch (index) {
    case 0: return ExternalParentVariantChild.load(deserializer);
    default: throw new Error("Unknown variant index for ExternalParent: " + index);
  }
}
}


export class ExternalParentVariantChild extends ExternalParent {

constructor (public value: ExternalChild) {
  super();
}
...
static load(deserializer: Deserializer): ExternalParentVariantChild {
  const value = ExternalChild.deserialize(deserializer);
  return new ExternalParentVariantChild(value);
}
