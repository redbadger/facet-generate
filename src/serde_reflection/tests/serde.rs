...
use crate::serde_reflection::{
    error::Error,
    format::{ContainerFormat, Format, FormatHolder as _, Named, VariantFormat},
    trace::{Samples, Tracer, TracerConfig},
    value::Value,
};
...
fn test_tracers() {
    ...
    let ContainerFormat::Enum(variants) = format else {
        unreachable!();
    };
...
fn test_trace_deserialization_with_recursive_types() {
    ...
    let ContainerFormat::Enum(variants) = registry.get("List").unwrap() else {
...
fn test_mixed_tracing_for_multiple_enums() {
    ...
    let registry = tracer.registry().unwrap();
    // Note that we do not use the type parameter in the name.
    let ContainerFormat::Enum(variants) = registry.get("Foo").unwrap() else {
        panic!("should be an enum")
    };
    assert_eq!(variants.len(), 2);
}
