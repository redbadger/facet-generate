...
- **Generation rejects a namespace that collides with a type, the root package or another namespace**: each installer now checks, before writing anything, that every module gets its own identifier and file, and that no type in scope hides a namespace's qualifier. Each of these used to produce code that didn't compile, or lost a module's source when two modules were written to one file. They now fail with an `InvalidInput` error naming both sides and suggesting `#[facet(rename = "...")]` or a different namespace. That covers a root `Kv` beside namespace `kv` (Swift, TypeScript, C#); a namespace `Kit` beside a class `Kit` (Kotlin, C#); namespaces that case-convert to the same module or to the root package's (`app` in package `App`, or `kv` and `Kv`), with file names compared case-insensitively on every platform; a type named like the first segment of a qualified reference, such as `Example` in package `Example`; and, in C#, a namespace named after a builtin the generated code writes bare (`unit` becoming `Example.Unit`). A namespace spelled exactly like the root package is still merged into it [#153](https://github.com/redbadger/facet-generate/issues/153), [#184](https://github.com/redbadger/facet-generate/pull/184)
- **TypeScript and Swift also reject a namespace that shadows a builtin**, as C# does for `unit`. In TypeScript, a namespace can't be imported as a global that the plugins' code constructs in that module: `Map` (for a map field), or `Error` or `Uint8Array` (for an enum or a UUID). A module named `serde` is also rejected when a plugin installs the `serde/` runtime, because `./serde` would find the module instead. In Swift, a namespace can't become a module named like a standard-library type, such as `String`, `Error` or `Result`. Swift looks a qualifier up as a type first, so `String.Foo` means `Swift.String.Foo` and the module can't be qualified at all. Swift also rejects a module named `Swift` or `Serde`, or one named after an SDK module that `Foundation` loads (`System`, `Darwin`, `Combine` and others) when the package imports `Foundation`. A target of that name makes a module dependency cycle. Each of these used to produce code that didn't compile. Now generation fails with #153's `InvalidInput` error [#185](https://github.com/redbadger/facet-generate/issues/185), [#187](https://github.com/redbadger/facet-generate/pull/187)
- **Generation rejects a package name that is also an external package's namespace.** A namespace spelled exactly like the root package is merged into the root module. So with package `shared` and an external namespace `shared`, the module would be both generated and provided externally. Until now, TypeScript and Swift silently dropped the ROOT types, and C# and Kotlin wrote the external types a second time. All four installers now fail with an `InvalidInput` error before writing anything [#186](https://github.com/redbadger/facet-generate/issues/186), [#194](https://github.com/redbadger/facet-generate/pull/194)
- **Kotlin JSON now matches the JSON Rust reads and writes.** The Kotlin `JsonPlugin` used kotlinx.serialization's own conventions. Properties were keyed by their camelCase names. Data enums were written as `{"type":"Variant",…}`. Newtypes were `{"value":x}`, and tuples and tuple structs were objects. A null optional field was left out, and bytes were signed. So Kotlin couldn't exchange JSON with a Rust `serde_json` core beyond flat structs. It now writes serde_json's format, as #157 did for Swift. Every property is keyed by the registry's wire name with `@SerialName`, and data enums follow the registry's tagging: external (`{"Variant":…}`), internal (`#[facet(tag)]`) or adjacent (`#[facet(tag, content)]`). Newtype structs are their inner value, and tuples and tuple structs are arrays. Unit structs and `()` are `null`, an optional field is written as `null`, bytes are numbers from 0 to 255, and 64- and 128-bit integers are exact. Types kotlinx can't derive this way get a nested `JsonSerializer`, built on a new runtime file, `com/novi/serde/JsonCoding.kt`. An external serde package must now provide that file too. A variant class is no longer `@Serializable` on its own: encode it through its enum. Structs with an optional field opt in to `ExperimentalSerializationApi` for `@EncodeDefault`. serde ignores unknown keys, so configure your `Json` with `ignoreUnknownKeys = true` to match it. **JSON stored or exchanged in the old format won't decode.** Kotlin plugins now get the real container in `type_annotations` and `type_body`, and Kotlin now calls `field_annotations`. A round trip against serde_json covers every shape [#182](https://github.com/redbadger/facet-generate/issues/182), [#195](https://github.com/redbadger/facet-generate/pull/195)
- **C# JSON now matches the JSON Rust reads and writes.** The C# `JsonPlugin` put `[JsonPropertyName]` on the private backing field, which System.Text.Json ignores, so properties were written in PascalCase. Data enums were written as `{"type":"Variant",…}`. Newtypes were objects, `byte[]` was base64, and renamed unit variants weren't honoured. Every generated type now carries `[JsonConverter(typeof({Type}JsonConverter))]`, and a converter written beside the type writes serde_json's format. Fields are keyed by the registry's wire names, and data enums follow its tagging: external, internal or adjacent. Newtypes are their inner value, and tuples and tuple structs are arrays. Unit structs and `()` are `null`, and an optional field is written as `null`. Bytes are numbers from 0 to 255, 64- and 128-bit integers are exact, and non-finite floats are `null`. Map keys are stringified as serde_json does them, including integer, bool and newtype keys. Unknown keys are ignored, as in serde, and no `JsonSerializerOptions` are needed. The converters ignore naming policies. The runtime gains `FacetJson.cs`, with the public types `FacetJson`, `JsonEnum` and `JsonVariantReader`. `JsonSerde.Options` no longer registers `JsonStringEnumConverter`. A variant record serialized by its own type bypasses the converter, so serialize it through its enum. **JSON stored or exchanged in the old format won't decode.** C# plugins' `type_annotations` is now called for structs and unit structs as well. A round trip against serde_json covers every shape [#182](https://github.com/redbadger/facet-generate/issues/182), [#196](https://github.com/redbadger/facet-generate/pull/196)
- **TypeScript JSON now works, and matches the JSON Rust reads and writes.** The TypeScript `JsonPlugin` used to generate bincode's positional `serialize(serializer)` calls, and installed no JSON runtime, so its output couldn't produce or read JSON at all. Each class now has static `toJson` and `fromJson`, which convert to and from serde_json's shapes, and `jsonSerialize` and `jsonDeserialize`, which go to and from text. Each enum has the free functions `toJson{E}`, `fromJson{E}`, `jsonSerialize{E}` and `jsonDeserialize{E}`. `JSON.stringify` and `JSON.parse` do the text. Fields are keyed by the registry's wire names, and data enums follow its tagging: external, internal or adjacent. Newtypes are their inner value, and tuples and tuple structs are arrays. Unit structs and `()` are `null`, bytes are numbers from 0 to 255, and map keys are stringified as serde_json does them. Unknown keys are ignored. 64- and 128-bit integers are exact. They're read through the `JSON.parse` reviver's `context.source` and written with `JSON.rawJSON`, **which need Node 21+, Deno 1.37+, Chrome 114+, Safari 18.4+ or Firefox 135+**. On an older engine, only a value beyond 2^53 throws, with an error saying why. The plugin installs a new runtime file, `serde/json.ts`, in place of the bincode serde files. An external serde package must provide `json.ts` too. Separately, a field whose wire name isn't a TypeScript identifier, such as `with-dash`, is now declared as a quoted property, whichever plugin is used. That output didn't compile before, and bincode's code for such a field still doesn't. A round trip against serde_json covers every shape [#175](https://github.com/redbadger/facet-generate/issues/175), [#198](https://github.com/redbadger/facet-generate/pull/198)
- **Swift and TypeScript reject more namespaces that shadow a name in scope**, extending the #185 entry above. In Swift, a module can't be named like a type that Foundation brings into scope, such as `Data`, `Date`, `Locale` or `DispatchQueue`. That applies only in a module that imports `Foundation` itself, which is one with a UUID. The 366 names were generated from the SDK and checked against the compiler. A Swift module also can't be named like a type in the `Serde` runtime that its plugins install, such as `Serializer` with bincode or `JsonKey` with JSON. That list is read from the runtime sources, so it can't drift. The standard-library list now includes `_StringProcessing`, which every Swift file imports, so `regex` is rejected. In TypeScript, a namespace can't be imported under a name the module already imports or declares, such as `Serializer` from the plugin's runtime, the exported `Uuid` alias, or bincode's `HEX` helper. That's checked against what the module actually contains. Each of these used to produce code that didn't compile [#192](https://github.com/redbadger/facet-generate/issues/192), [#203](https://github.com/redbadger/facet-generate/pull/203)
- **Kotlin: tuples of four to twelve elements are the runtime's `Tuple4` to `Tuple12`, and a longer one is rejected.** A tuple of four or more elements was written as an `NTupleN` that nothing declared, so it didn't compile with either plugin. It's now the serde runtime's `TupleN`, which a module imports only when it holds a tuple of that length. The runtime already had `Tuple4` to `Tuple6`, and gains `Tuple7` to `Tuple12`. `JsonCoding.kt` gains `JsonTuple4Serializer` to `JsonTuple12Serializer`, which write a tuple as an array, as serde_json does. An external serde package must now provide the new files and serializers too. Twelve is as far as `facet` reflects a tuple (with its `tuples-12` feature, and four without it), so only a hand-built registry has a longer one. That now fails with an `InvalidInput` error naming the type that holds it, before anything is written. A type named `Tuple4` to `Tuple12`, or `JsonTuple4Serializer` to `JsonTuple12Serializer`, is rejected only in a module that holds a tuple of that length, and `NTuple4` to `NTuple6` are no longer reserved. `Feature` gains `Tuple(usize)`. Runtime round trips against Rust bincode and serde_json cover a 4- and a 12-tuple [#129](https://github.com/redbadger/facet-generate/issues/129)
- **C#: a `char` is now declared as a `string`**, under every plugin and with none. A C# `char` is one UTF-16 code unit, so it can't hold a Rust `char` outside the BMP, such as an emoji. A `string` can hold any Unicode scalar value. Bincode and JSON throw if it holds anything but exactly one: bincode throws a `SerializationError` when writing and a `DeserializationError` when reading, and JSON throws a `JsonException` both ways. JSON still writes a `char` as a string holding that one scalar, as `serde_json` does, as a value and as an object key. An `Option<char>` is now a `string?`, a reference type, where it was a `char?`. Code that sets or reads these properties needs strings in place of `char`s, `"a"` for `'a'`. Each module with a `char` declares the helper classes `CharSerde` (bincode) and `JsonCharConverter` (JSON), so generation rejects a type of either name in a registry with a `char` [#213](https://github.com/redbadger/facet-generate/issues/213)
...
- **C#: a property named like a type no longer hides it in a static call**: every field is a property, so a field `presence` is a property `Presence`. In a class's static `Deserialize`, a bare `Presence.Deserialize(...)` then meant the property, not the type, and failed with CS0120. That happened whenever the property had any other type, such as `Vec<Presence>`. The same went for an enum's `…Bincode` helper, `FacetHelpers`, `UuidSerde` and the JSON plugin's `JsonSerde`, on both the serialize and the deserialize side, including in a data enum's variants. Such a call is now written through its `global::` name, but only where a property or variant actually hides it, so output that already compiled is unchanged [#159](https://github.com/redbadger/facet-generate/issues/159), [#177](https://github.com/redbadger/facet-generate/pull/177)
- **C#: a data enum's variant named like a type it holds, or like a sibling's field, now compiles**. Each variant is a record nested in the enum's base record, so inside the enum a variant's name hides any type or property of the same name. In `enum Event { Presence(Presence) }`, `Presence(Presence Value)` meant the variant itself and failed with CS8910. A field of that type in another variant silently bound to the variant record instead of the struct, or failed with CS1503 under bincode. A sibling field named `presence` failed with CS8866, and so did a variant named `Value` or `Field0` beside a newtype or tuple variant. Now a type named like a variant is written through its `global::` name, and a property named like one is declared again with `new`, so it keeps its name. Records with no clash are unchanged. The only existing output that changes had failed with CS8866: a struct variant's field named like a sibling variant, such as `default` beside `Default` [#174](https://github.com/redbadger/facet-generate/issues/174), [#188](https://github.com/redbadger/facet-generate/pull/188)
- **C# and Kotlin: the root module is no longer skipped when the package name ends in an external namespace**. The installers skipped any module whose name ended in a namespace listed in `external_packages`. The root module's name is the package name, so with package `Acme.shared` (C#) or `com.acme.shared` (Kotlin) and an external `shared`, the ROOT types were silently not written. They now skip a module only when its namespace is external [#186](https://github.com/redbadger/facet-generate/issues/186), [#194](https://github.com/redbadger/facet-generate/pull/194)
- **C#: an external NuGet package is named by its package ID, not the last segment of its URL**. `https://www.nuget.org/packages/Acme.Contracts/2.4.1` used to become `<PackageReference Include="2.4.1" …>`, and a URL with a trailing slash was named after the namespace. The installer now reads the ID and version from nuget.org's gallery, v2 API and `.nupkg` URLs. `ExternalPackage::module_name`, if set, names the package explicitly for C#. The version is `ExternalPackage::version`, then the URL's version, then `1.0.0` as before [#186](https://github.com/redbadger/facet-generate/issues/186), [#194](https://github.com/redbadger/facet-generate/pull/194)
- **Kotlin and C#: a root package that ends in a namespace's name no longer loses a segment**: a reference was taken to be the module's own when its namespace matched the last segment of the module's name. So with package `com.kv` and namespace `kv`, the root module wrote `com.kv.Entry` for `com.kv.kv.Entry`, and C# with package `Example.kv` wrote a bare `Entry`. Neither compiled. The generators now decide from the module's `namespace` (see `CodeGeneratorConfig::generates`), and `kotlin::requalify` and `csharp::requalify` follow. A config built by hand with `CodeGeneratorConfig::new` and no parent is now always the root module; give a namespaced module `with_parent`, as the installers do [#164](https://github.com/redbadger/facet-generate/issues/164), [#178](https://github.com/redbadger/facet-generate/pull/178)
- **Swift: `Hashable` and `Equatable` are decided over the whole registry**: each module worked them out from its own types and assumed every type from another module conformed to both. So a root `SwHash` holding a `kit` type with a tuple field was declared `Hashable` and didn't build. The installer now decides conformance once for the whole registry, and every module uses the same answer. Two older bugs within a single module are fixed too: a type in a cycle could be declared `Hashable` depending on which type was checked first, and a holder of `Vec<(u32, u32)>` was declared `Equatable` although the array isn't. Types from external packages are still assumed to conform [#156](https://github.com/redbadger/facet-generate/issues/156), [#179](https://github.com/redbadger/facet-generate/pull/179)
- **Swift JSON builds, and reads and writes the JSON Rust does**: the Swift `JsonPlugin` called a `JsonSerializer` and `JsonDeserializer` that no runtime defined, so its output never built. It also wrote the same positional calls as bincode, with no field or variant names, so no serializer could have produced JSON that Rust reads. Swift types now conform to `Codable`, with `CodingKeys` from the registry's wire names and hand-written coding where Swift's automatic coding differs from `serde_json`. Enums are externally tagged unless `#[facet(tag)]` or `#[facet(tag, content)]` says otherwise, tuples and tuple structs are arrays, newtype structs are transparent, `Option` is `null`, maps with non-string keys use string keys, and `u128`/`i128` are exact numbers. `jsonSerialize()` and `jsonDeserialize(input:)` keep their signatures and wrap `JSONEncoder`/`JSONDecoder`. A new runtime round trip against `serde_json` covers every variant kind, the integer extremes, maps and recursive types. Two gaps remain: an empty braced struct or variant (`S {}`) is written as `null` or `"V"`, where Rust expects `{}`, because the registry doesn't record the braces; and a non-finite float can't be encoded [#157](https://github.com/redbadger/facet-generate/issues/157), [#180](https://github.com/redbadger/facet-generate/pull/180)
- **Two different types with the same name in two namespaces are no longer ambiguous**: the namespace-ambiguity check compared bare names, so a `Child` explicitly in namespace `a` and a different `Child` inherited into `b` failed with `AmbiguousNamespaceInheritance`. It now compares the Rust types, and both generate in their own modules. One type reached under two namespaces still errors [#138](https://github.com/redbadger/facet-generate/issues/138), [#183](https://github.com/redbadger/facet-generate/pull/183)
...
- **TypeScript bincode: a field whose wire name isn't an identifier now compiles**. With the field declared as a quoted property (see the TypeScript JSON entry), bincode's `serialize` still wrote `this.with-dash`, which reads as `this.with - dash`. A struct variant's `deserialize` returned `{ kind: "Other", with-dash }`, which doesn't parse. Both now write the name through `this["with-dash"]` and `"with-dash": with_dash`. Output for identifier names is unchanged [#197](https://github.com/redbadger/facet-generate/issues/197), [#199](https://github.com/redbadger/facet-generate/pull/199)
- **TypeScript: a `[T; N]` field now compiles in a module with no tuple**. The `ListTuple` alias that such a field is written through was defined through `Tuple`. A module declares `Tuple` only when it holds a real tuple, so without one it failed with TS2304 ("Cannot find name 'Tuple'"), with any plugin or none. `ListTuple<T>` is now `T[]` directly, the same type. Bincode's reader for such a field also failed to type-check (TS2322), because `[item]` is inferred as `number[]`, not `[number]`, and it now writes `[item] as [T]`. The TypeScript type of `[T; N]` is unchanged [#190](https://github.com/redbadger/facet-generate/issues/190), [#200](https://github.com/redbadger/facet-generate/pull/200)
- **A `Uuid` field now compiles without a plugin**. TypeScript declared the `Uuid` alias only in the plugins (TS2304). Swift imported `Foundation` only in the plugins ("cannot find type 'UUID' in scope"). Kotlin imported `java.util.UUID` only in the plugins. Now TypeScript's emitter declares `Uuid`, Swift's emitter imports `Foundation` whenever a module has a UUID, and Kotlin's emitter imports `java.util.UUID` when no plugin provides it. C# already built. TypeScript's Bincode and JSON plugins on one module now build as well: both used to declare `export type Uuid` (TS2300). The Swift plugins no longer add `Foundation` to their imports. Swift's check for a module named like an SDK module now also applies without a plugin. It rejects `foundation` when any module imports `Foundation`, and `system` and the others when that module imports `Foundation` itself, through the runtime, or through a module it imports [#191](https://github.com/redbadger/facet-generate/issues/191), [#201](https://github.com/redbadger/facet-generate/pull/201)
- **C#: a variant's property named like the variant is renamed, not rejected**. A struct variant's field named like its own variant, such as `Presence { presence }` or `Status { status: u8 }`, was rejected at generation. C# doesn't allow a member named like its enclosing type (CS0542), and the variant is a record of that name. The same went for a newtype variant named `Value` and a tuple variant named `Field0`. Such a property now gets a trailing underscore, as in `record Presence(uint Presence_)`. A Rust field can never produce that spelling, so it can't clash with a sibling field. The JSON key is still the wire name (`presence`), and output for every other variant is unchanged. A top-level type's member named like the type is still rejected [#193](https://github.com/redbadger/facet-generate/issues/193), [#202](https://github.com/redbadger/facet-generate/pull/202)
- **Kotlin: Bincode and JSON on one module now build with a `Uuid` or bytes field**. Bincode imported `java.util.UUID` and `com.novi.serde.Bytes`, and JSON declares its own `UUID` and `Bytes` aliases, annotated with its serializers. An explicit import outranks a declaration in the same package, so the `@Serializable` types saw the bare types and failed with "Serializer has not been found for type 'UUID'" (or `'Bytes'`). Kotlin's emitter now imports `java.util.UUID` itself, and leaves out any import of a name that a plugin declares. Plugins declare such names with a new default method, `EmitterPlugin::module_declarations`. Bincode's code uses JSON's aliases, which are the same types. Output with a single plugin, or none, is unchanged [#204](https://github.com/redbadger/facet-generate/issues/204)
- **Kotlin bincode: 128-bit integers, `char`s and fixed-size arrays now compile, and write what Rust's `bincode` does**. A `u128` or `i128` is declared as a `BigInteger`, but was serialized as the runtime's `UInt128` / `Int128`. A `char` is declared as a `String`, since a Kotlin `Char` can't hold one outside the BMP such as an emoji, but was serialized as a `Char`, which the runtime doesn't implement. A `[T; N]` was passed to the `List<T>` helper without its element lambda, which would also have written a length that bincode doesn't. Each module now gets private helpers for the formats it holds: a `BigInteger` is converted to and from `Int128` / `UInt128`, and one out of the Rust type's range throws a `SerializationError`. A `char` is written as its UTF-8 bytes with no length, as bincode 1 does, a string that isn't exactly one Unicode scalar value throws a `SerializationError`, and bytes that aren't one throw a `DeserializationError`. A `[T; N]` is written element by element. The serde runtime is unchanged, so an external serde package needs nothing new. `Feature` has a new `Char` variant. Output with the JSON plugin, or none, is unchanged [#127](https://github.com/redbadger/facet-generate/issues/127)
- **TypeScript bincode: a nested tuple, or two tuples in one type, now compiles**. `deserialize` read a tuple's elements into `const field0`, `const field1`, … in the enclosing scope. So a tuple inside a tuple, a second tuple field, a tuple map key beside a tuple value, or a tuple inside a tuple variant declared them again (TS2451), and an outer tuple was built from its inner tuple's elements (TS2352). Each tuple element now takes the first `field{n}` that nothing else in its scope uses, so `(u8, (String, bool))` reads into `field0`, then `field2` and `field3` for the inner tuple, which becomes `field1`. A field of the type that is itself named `field0` keeps its name. Output for a type with one flat tuple, and no field named `field{n}`, is unchanged [#211](https://github.com/redbadger/facet-generate/issues/211)
- **C# JSON: tuples of eight to twelve elements now compile**. The JSON converters build a tuple's converter with the runtime's `FacetJson.Tuple`, whose overloads stopped at seven elements, so a longer tuple failed with CS1501 ("No overload for method 'Tuple' takes 12 arguments"). `FacetJson.Tuple` now takes up to twelve, the most facet reflects. C#'s `ValueTuple` nests the elements past the seventh in its `TRest`, but the JSON is still one flat array of every element, as `serde_json` writes it. Bincode already built these tuples and round trips them. An external serde package that ships its own copy of the runtime needs the updated `FacetJson.cs`, but only for tuples of eight or more elements [#212](https://github.com/redbadger/facet-generate/issues/212)
- **Bincode: C#, Swift and TypeScript now write a `char` as Rust's `bincode` does**. Rust's `bincode` writes a `char` as its UTF-8 bytes, one to four of them, with no length prefix. C#'s runtime wrote a `u32` instead, and Swift's and TypeScript's threw "not implemented", so no `char` but a `None` round-tripped with Rust. Each module with a `char` now gets private helpers that write and read its UTF-8 bytes through the runtime's `u8` methods: C#'s `CharSerde` class, extensions of Swift's `Serializer` and `Deserializer` protocols, and TypeScript's `writeChar` and `readChar` functions. A value that isn't exactly one Unicode scalar value throws on serialize, such as a Swift `Character` of `e` and a combining accent, or a TypeScript string of two. Bytes that aren't the UTF-8 encoding of one throw on deserialize. The serde runtimes are unchanged, so an external serde package needs nothing new, and generated code no longer calls their `char` methods. The TypeScript helpers throw an `Error`, so a namespace imported as `Error` is now rejected in a module with a `char`, as it already was in one with an enum or a UUID. Swift JSON now checks the same thing. `Serde.JsonChar` counted grapheme clusters, so it read `e` and a combining accent, which `serde_json` rejects, and it wrote any `Character`. Swift JSON now goes through a private adapter, generated beside the types, that reads and writes exactly one Unicode scalar value [#213](https://github.com/redbadger/facet-generate/issues/213)

### 🧪 Tests

- **Swift builtins shadowed by an imported namespace ([#152](https://github.com/redbadger/facet-generate/issues/152))**: `swift build` tests cover a namespaced `Set` beside a `HashSet` field and a namespaced `String` beside a `String` field, with no plugin, bincode and JSON, checking that the root module writes `Swift.Set` and `Swift.String`. The fix was already in [#155](https://github.com/redbadger/facet-generate/pull/155) [#183](https://github.com/redbadger/facet-generate/pull/183)
