...
class SerdeTests: XCTestCase {
    ...
    func testULEB128Encoding() throws {
        ...
        XCTAssertEqual(
            serializer.get_bytes(), [0, 1, 127, 128, 1, 184, 23], "the array should be same")
    }
    ...
    func testCheckThatKeySlicesAreIncreasing() throws {
        ...
        XCTAssertNoThrow(
            try d.check_that_key_slices_are_increasing(
                key1: Slice(start: 0, end: 3), key2: Slice(start: 3, end: 5)))
        XCTAssertThrowsError(
            try d.check_that_key_slices_are_increasing(
                key1: Slice(start: 0, end: 3), key2: Slice(start: 0, end: 3)))
        XCTAssertThrowsError(
            try d.check_that_key_slices_are_increasing(
                key1: Slice(start: 1, end: 3), key2: Slice(start: 3, end: 5)))
    }
