...
//!
//! Provides JSON-specific imports, feature helper snippets, and full
...
//! | `type_body` | `serialize` / `deserialize` methods + `jsonSerialize` / `jsonDeserialize` wrappers |
//!
//! # JSON struct-field tuple serialisation
//!
//! For a struct field `foo: (A, B)`, JSON wraps the elements in an extra
//! container-depth push/pop, using the field name (without `self.`) as the
//! tuple receiver:
//! ```swift
//! try serializer.increase_container_depth()   // tuple-level
//! try serializer.serialize_a(value: foo.0)
//! try serializer.serialize_b(value: foo.1)
//! try serializer.decrease_container_depth()
//! ```
//! Combined with the struct-level push/pop this produces double nesting.
...
/// Serialise one struct field, applying JSON tuple wrapping when the field
/// type is a native tuple.
fn write_field_serialize(w: &mut dyn IndentWrite, fname: &str, format: &Format) -> io::Result<()> {
    match format {
        // JSON wraps native tuple fields in an extra container-depth level.
        // The receiver is the field name (no `self.` prefix) so elements are
        // addressed as `fname.0`, `fname.1`, …
...
/// Deserialise one struct field, applying JSON tuple wrapping when the field
/// type is a native tuple.
fn write_field_deserialize(
    ...
    match format {
        // JSON wraps native tuple fields in an extra container-depth level.
        // Intermediate variables are named `fname0`, `fname1`, … (no `Field`
        // infix, matching the original emitter behaviour).
        Format::Tuple(formats) => {
            push_deserializer(w)?;
            for (i, fmt) in formats.iter().enumerate() {
...
// ---------------------------------------------------------------------------
// Enum type body (identical structure to BincodePlugin)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Variant helpers (identical to BincodePlugin)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Format serialisation helpers (identical to BincodePlugin)
// ---------------------------------------------------------------------------
