...
/// `Kit.Presence.deserialize(…)`), which did not compile (#154). The `kit`
/// type guards the case that always worked.
///
...
///
/// Snapshots only: the references themselves are still broken in every
/// language, #148 (Kotlin), #149 (C#), #150 (TypeScript) and #151 (Swift).
#[test]
