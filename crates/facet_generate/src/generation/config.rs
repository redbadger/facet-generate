...
pub struct CodeGeneratorConfig {
    ...
    /// The namespace whose types this module generates: [`Namespace::Root`]
    /// for the module named after the package, and `Named(ns)` for the module
    /// of namespace `ns`.
    ///
    /// Set by [`module::split`](super::module::split), which every installer
    /// splits the registry with, and [`Namespace::Root`] in a config made with
    /// [`new`](Self::new). Kept because the module name cannot be decomposed
    /// reliably (see [`parent`](Self::parent)); ask it with
...
impl CodeGeneratorConfig {
    ...
    /// Whether this module is the one that generates the type `name`, in
    /// registry spelling — whether `name` lives in this module's
    /// [`namespace`](Self::namespace).
    ///
    /// A ROOT type is generated by the module named after the package, and a
    /// type in namespace `ns` by the module of `ns`, whatever the language
    /// calls that module (`ns` in Swift and TypeScript, `<package>.ns` in
    /// Kotlin and C#). A type in a namespace named after the package itself
    /// shares the root module, as [`module::split`](super::module::split)
    /// puts it there. The answer does not depend on whether the registry has
    /// a type called `name`.
    ///
    /// # Examples
    ///
    /// ```
    /// use facet_generate::{
    ///     generation::{CodeGeneratorConfig, module},
    ///     reflection::format::{ContainerFormat, Doc, QualifiedTypeName},
    ///     Registry,
    /// };
    ///
    /// let presence = QualifiedTypeName::namespaced("kit".into(), "Presence".into());
    /// let registry = Registry::from([(presence.clone(), ContainerFormat::UnitStruct(Doc::new()))]);
    ///
    /// for (module, _) in module::split("shared", &registry) {
    ///     let config = module.config().clone().with_parent("shared");
    ///     assert_eq!(config.module_name(), "shared.kit");
    ///     assert!(config.generates(&presence));
    ///     assert!(!config.generates(&QualifiedTypeName::root("Event".into())));
    /// }
    /// ```
    #[must_use]
    pub fn generates(&self, name: &QualifiedTypeName) -> bool {
        match (&self.namespace, &name.namespace) {
            (Namespace::Root, Namespace::Named(ns)) => ns == self.root_package(),
            (own, theirs) => own == theirs,
        }
    }
    ...
    /// Records references to `names`, in registry spelling, on top of those
    /// the module's registry makes — the ones a plugin declares through
    /// [`referenced_types`](super::plugin::EmitterPlugin::referenced_types).
    ///
    /// A type in a named namespace other than the module's own is recorded as
    /// [`update_from`](Self::update_from) records a reference in the registry
    /// ([`referenced_namespaces`](Self::referenced_namespaces)) and as
    /// [`module::split`](super::module::split) does
    /// ([`external_definitions`](Self::external_definitions)); both are sets
    /// in effect, so a namespace the registry references already is not added
    /// twice. A ROOT type is left to the language, since whether it lives in
    /// another module depends on how the language lays modules out.
    pub(crate) fn reference_types(&mut self, names: &[QualifiedTypeName]) {
        for name in names {
            if let Namespace::Named(ns) = &name.namespace
                && ns != &self.module_name
            {
                self.referenced_namespaces.insert(ns.clone());
                let known = self.external_definitions.entry(ns.clone()).or_default();
                if !known.contains(&name.name) {
                    known.push(name.name.clone());
                }
            }
        }
    }
...
/// Every type the formats of `registry` reference, in registry spelling.
pub(crate) fn registry_references(registry: &Registry) -> BTreeSet<QualifiedTypeName> {
