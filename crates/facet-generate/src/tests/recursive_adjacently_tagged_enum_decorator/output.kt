...
sealed interface Options {
    @Serializable @SerialName("red") data class Red(val content: Boolean) : Options

    @Serializable @SerialName("banana") data class Banana(val content: String) : Options

    @Serializable @SerialName("vermont") data class Vermont(val content: Options) : Options
}

@Serializable
@JsonClassDiscriminator("type")
sealed interface MoreOptions {
    @Serializable @SerialName("news") data class News(val content: Boolean) : MoreOptions
    ...
    data class Exactly(val config: String) : MoreOptions {
        object Serializer : KSerializer<Exactly> {
            @Serializable private data class Content(val config: String)

            override val descriptor =
                    buildClassSerialDescriptor("exactly") { element<Content>("content") }
    ...
    data class Built(val top: MoreOptions) : MoreOptions {
        object Serializer : KSerializer<Built> {
            @Serializable private data class Content(val top: MoreOptions)

            override val descriptor =
                    buildClassSerialDescriptor("built") { element<Content>("content") }
