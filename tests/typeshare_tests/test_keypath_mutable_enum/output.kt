...
sealed interface InternallyTagged: KeyPathMutable<InternallyTagged> {
    @Serializable
    @SerialName("unit")
    data object Unit : InternallyTagged
    ...
    @Serializable
    @SerialName("emptyStruct")
    data object EmptyStruct : InternallyTagged

    @Serializable
    @SerialName("tuple")
    data class Tuple(val value: String) : InternallyTagged

    object Serializer : KSerializer<InternallyTagged> {
        override val descriptor = buildClassSerialDescriptor("InternallyTagged") {
        ...
        override fun serialize(encoder: Encoder, value: InternallyTagged) {
            ...
            when (value) { 
                ...
                is AnonymousStruct -> {
                    val content = encoder.json.encodeToJsonElement(AnonymousStruct.serializer(), value) as JsonObject
                    val base = mutableMapOf<String, JsonElement>("type" to JsonPrimitive("anonymousStruct"))
                    val element = JsonObject(base.apply { putAll(content.toMap()) })
                    encoder.encodeJsonElement(element)
                }
                is EmptyStruct -> {
                    val content = encoder.json.encodeToJsonElement(EmptyStruct.serializer(), value) as JsonObject
                    val base = mutableMapOf<String, JsonElement>("type" to JsonPrimitive("emptyStruct"))
                    val element = JsonObject(base.apply { putAll(content.toMap()) })
                    encoder.encodeJsonElement(element)
                }
                is Tuple -> {
    ...
    override fun patching(patch: PatchOperation, keyPath: List<KeyPathElement>): InternallyTagged {
        ...
        return when {
            this is Unit && variant == KeyPathElement.Variant("unit", VariantTagType.INTERNAL) -> {
                throw IllegalStateException("InternallyTagged.Unit does not support nested key paths")
            }
            this is AnonymousStruct && variant == KeyPathElement.Variant("anonymousStruct", VariantTagType.INTERNAL) -> {
                when (field) {
                    ...
                    else -> throw IllegalStateException("InternallyTagged.AnonymousStruct does not support $field key path")
                }
            }
            this is EmptyStruct && variant == KeyPathElement.Variant("emptyStruct", VariantTagType.INTERNAL) -> {
                throw IllegalStateException("InternallyTagged.EmptyStruct does not support nested key paths")
            }
            this is Tuple && variant == KeyPathElement.Variant("tuple", VariantTagType.INTERNAL) -> {
                val newValue = this.value.patching(patch, keyPath.drop(2))
                this.copy(value = newValue)
            }
            else -> throw IllegalStateException("InternallyTagged has no mutable $variant key path.")
        }
    }
    ...
    private fun applying(patch: PatchOperation): InternallyTagged {
        when (patch) {
            ...
            is PatchOperation.Splice -> throw IllegalStateException("InternallyTagged does not support splice operations.")
        }
    }
}
...
sealed interface ExternallyTagged: KeyPathMutable<ExternallyTagged> {
    @Serializable
    data class AnonymousStruct(val foor: Int, val bar: String) : ExternallyTagged

    @Serializable
    data class Tuple(val value: String) : ExternallyTagged

    object Serializer : KSerializer<ExternallyTagged> {
        override val descriptor = buildClassSerialDescriptor("ExternallyTagged") {
            element<AnonymousStruct>("anonymousStruct", isOptional = true)
            element<Tuple>("tuple", isOptional = true)
        }
        ...
        override fun serialize(encoder: Encoder, value: ExternallyTagged) {
            encoder.encodeStructure(descriptor) {
                when (value) { 
                    ...
                    is Tuple -> encodeSerializableElement(
                        descriptor,
                        1,
                        String.serializer(),
                        value.value
    ...
    override fun patching(patch: PatchOperation, keyPath: List<KeyPathElement>): ExternallyTagged {
        ...
        val field = keyPath[1]
        return when {
            this is AnonymousStruct && variant == KeyPathElement.Variant("anonymousStruct", VariantTagType.EXTERNAL) -> {
                when (field) {
                    KeyPathElement.Field("foor") -> {
                    ...
                    else -> throw IllegalStateException("ExternallyTagged.AnonymousStruct does not support $field key path")
                }
            }
            this is Tuple && variant == KeyPathElement.Variant("tuple", VariantTagType.EXTERNAL) -> {
                when (field) {
                    KeyPathElement.Field("0") -> {
                        val newValue = this.value.patching(patch, keyPath.drop(2))
                        this.copy(value = newValue)
                    }
                    else -> throw IllegalStateException("ExternallyTagged.Tuple does not support $field key path")
                }
            }
            else -> throw IllegalStateException("ExternallyTagged has no mutable $variant key path.")
        }
    }
    ...
    private fun applying(patch: PatchOperation): ExternallyTagged {
        when (patch) {
            is PatchOperation.Update -> return patch.value.asT()
            is PatchOperation.Splice -> throw IllegalStateException("ExternallyTagged does not support splice operations.")
        }
    }
}
...
enum class Unit: KeyPathMutable<Unit> {
    ...
    private fun applying(patch: PatchOperation): Unit {
        when (patch) {
            is PatchOperation.Update -> return patch.value.asT()
            is PatchOperation.Splice -> throw IllegalStateException("Unit does not support splice operations.")
        }
    }
}
...
sealed interface Generic<T>: KeyPathMutable<Generic<T>> {
    @Serializable
    data class AnonymousStruct<T>(val foo: T) : Generic<T>

    @Serializable
    data class Tuple<T>(val value: T) : Generic<T>

    object Serializer : KSerializer<Generic<T>> {
        override val descriptor = buildClassSerialDescriptor("Generic<T>") {
        ...
        override fun deserialize(decoder: Decoder): Generic<T> {
            return decoder.decodeStructure(descriptor) {
                when (val index = decodeElementIndex(descriptor)) { 
                    0 -> {
                        return@decodeStructure decodeSerializableElement(descriptor, 0, AnonymousStruct<T>.serializer())
                    }
        ...
        return when {
            this is AnonymousStruct && variant == KeyPathElement.Variant("anonymousStruct", VariantTagType.EXTERNAL) -> {
                when (field) {
                    ...
                    else -> throw IllegalStateException("Generic<T>.AnonymousStruct does not support $field key path")
                }
            }
            this is Tuple && variant == KeyPathElement.Variant("tuple", VariantTagType.EXTERNAL) -> {
                when (field) {
                    ...
                    else -> throw IllegalStateException("Generic<T>.Tuple does not support $field key path")
                }
            }
    ...
    private fun applying(patch: PatchOperation): Generic<T> {
        when (patch) {
            is PatchOperation.Update -> return patch.value.asT()
            is PatchOperation.Splice -> throw IllegalStateException("Generic<T> does not support splice operations.")
        }
    }
}
