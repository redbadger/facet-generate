...
@Serializable data class SomeNamedStruct(val a_field: String, val another_field: UInt)

@Serializable(with = SomeResult.Serializer::class)
sealed interface SomeResult {
    @Serializable data class Ok(val value: UInt) : SomeResult

    @Serializable data class Error(val value: String) : SomeResult

    object Serializer : KSerializer<SomeResult> {
        override val descriptor =
                buildClassSerialDescriptor("SomeResult") {
        ...
        override fun serialize(encoder: Encoder, value: SomeResult) {
            encoder.encodeStructure(descriptor) {
                when (value) {
                    is Ok ->
                            encodeSerializableElement(descriptor, 0, UInt.serializer(), value.value)
                    is Error ->
...
sealed interface SomeEnum {
    @Serializable data class A(val field1: String) : SomeEnum

    @Serializable data class B(val field1: UInt, val field2: Float) : SomeEnum

    @Serializable data class C(val field3: Boolean? = null) : SomeEnum

    @Serializable data class D(val value: UInt) : SomeEnum

    @Serializable data class E(val value: SomeNamedStruct) : SomeEnum

    @Serializable data class F(val value: SomeNamedStruct?) : SomeEnum

    object Serializer : KSerializer<SomeEnum> {
        override val descriptor =
                buildClassSerialDescriptor("SomeEnum") {
        ...
        override fun serialize(encoder: Encoder, value: SomeEnum) {
            encoder.encodeStructure(descriptor) {
                when (value) {
                    is A -> encodeSerializableElement(descriptor, 0, A.serializer(), value)
                    is B -> encodeSerializableElement(descriptor, 1, B.serializer(), value)
                    is C -> encodeSerializableElement(descriptor, 2, C.serializer(), value)
                    is D -> encodeSerializableElement(descriptor, 3, UInt.serializer(), value.value)
                    is E ->
                            encodeSerializableElement(
                                    descriptor,
                                    4,
